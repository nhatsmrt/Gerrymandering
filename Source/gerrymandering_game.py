...
class GerrymanderGame():
    ...
    def find_num_seats(self, partition, epsilon_size = 3):

        # Check number of parts:
        n_part_missing_or_extra = 0
        n_part = len(partition)
        n_part_missing_or_extra = -np.abs(self._n_districts - n_part)
        ...
        parts_size_min = min(parts_size)
        parts_size_max = max(parts_size)
        if parts_size_max - parts_size_min - epsilon_size > 0:
            size_dif = epsilon_size - parts_size_max + parts_size_min

        # Check if any districts are disconnected:
        n_disconnected_part = 0
        for part in partition:
            if len(part) != 0:
                subgrph = self._G.subgraph(part)
                if not nx.is_connected(subgrph):
                    n_disconnected_part -= 1

        penalty = n_part_missing_or_extra + n_empty_part + size_dif + n_disconnected_part
        if penalty != 0:
            return penalty
    ...
    def state_to_partition(self, state):
        partition = [list() for i in range(self._n_districts)]
        for i in range(len(state)):
            partition[int(state[i])].append(i)
        return partition
...
class GerrymanderGame_v2():
    ...
    def sample_state(self):
        ...
        for pos in range(self._n_units):
    ...
    def find_num_seats(self, partition, epsilon_size = 1):
    ...
    def state_to_partition(self, state):
        ...
        for node in range(state.shape[0]):
            ...
            partition[part_ind].append(node)
        return partition
